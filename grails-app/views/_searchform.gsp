    ...
    <div style="margin-right: 30px;">
      ...
      <form style="position: relative" action="/synonyme" onsubmit="doSubmit()" name="searchform">
        ...
        <g:if test="${params && params.q}">
          <input ${autofocus} ${directSearchAttributes} style="outline: none" onclick="selectSearchField()" onblur="leaveSearchField()" accesskey="s" aria-label="Suche" type="text" id="search-field" name="q" value="${StringTools.slashUnescape(params.q.encodeAsHTML())}"
        </g:if>
        <g:else>
          <input ${autofocus} ${directSearchAttributes} style="outline: none" onclick="selectSearchField()" onblur="leaveSearchField()" accesskey="s" aria-label="Suche" type="text" id="search-field" name="q" placeholder="${message(code:'homepage.search.default.term')}"
        </g:else>
