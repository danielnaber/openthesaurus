					...
					<g:if test="${wikipediaResult}">
                        ...
                        <g:each in="${wikipediaResult}" var="term">
                            ...
                            <g:elseif test="${i > 0 && i < wikipediaResult.size() - 1}">
                                <g:link action="search" params="${[q: StringTools.slashEscape(StringTools.normalizeParenthesis(term))]}">${term.encodeAsHTML()}</g:link><span class="d">&nbsp;&middot;</span>
                            </g:elseif>
                            <g:else>
                                <g:link action="search" params="${[q: StringTools.slashEscape(StringTools.normalizeParenthesis(term))]}">${term.encodeAsHTML()}</g:link>
                            </g:else>
                            <% i++; %>
                        </g:each>
